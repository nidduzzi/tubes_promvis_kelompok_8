...
class AppShellState extends State<AppShell> {
  ...
  loginWithStoredCredentials(NhostAuthClient? auth) async {
    try {
      Logger.talker.log("loging in with stored credentials");
      // this will fetch refresh token and will sign user in!
      final res = await auth?.signInWithStoredCredentials();
      Logger.talker.log('user is signed in $res');
      return res;
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
        future: loginWithStoredCredentials(NhostAuthProvider.of(context)),
          ...
          if (!(snapshot.connectionState == ConnectionState.done)) {
            ...
            switch (auth?.authenticationState) {
              ...
              case AuthenticationState.signedOut:
                final locationRoute = AppRouter.routes
                    .where((element) => element.path == location);
          ...
          return Scaffold(
              ...
              appBar: AppBar(
                title: Text(selectedIndex != null
                    ? routeData[selectedIndex].title
                    : "App"),
              ),
              drawer: Drawer(
                child:
                    AppNavbar(routeData: routeData, scaffoldKey: scaffoldKey),
              ),
              body: TalkerWrapper(
